defmodule FilmChecklistWeb.UserAuthTest do
  use FilmChecklistWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias FilmChecklist.Accounts
  alias FilmChecklist.Accounts.Scope
  alias FilmChecklistWeb.UserAuth

  import FilmChecklist.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FilmChecklistWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FilmChecklistWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FilmChecklistWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
